...
import { getDetailCinemas } from '@/services/cinema/cinemaService';
import { useQuery } from '@tanstack/react-query';
import { Button } from 'antd';
import ServerError from '../../_components/500';
import { useRoomsCinemaMutation } from '../hooks/useRoomsCinema';

const CreateRoomByCinema = () => {
    const { handleSubmit, formState: { errors }, onMutationRooms, register, idCinema, data: screens, isPending, isLoading, isError } = useRoomsCinemaMutation({ type: "CREATE" });
    const { data: cinema, isLoading: isLoadingCinema, isError: isErrorCinema } = useQuery({
        queryKey: ['CINEMAS', idCinema],
        queryFn: async () => {
            return await getDetailCinemas(+idCinema!);
        }
    });
        ...
        <>
            <div className="card shadow mb-4">
                <div className="card-header py-3">
                    <h6 className="m-0 font-weight-bold text-primary text-uppercase">Tạo mới phòng chiếu cho rạp {cinema.data.cinema.name}</h6>
                </div>
...
export default CreateRoomByCinema